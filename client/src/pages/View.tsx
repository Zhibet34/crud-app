...
function View() {
    ...
    const searchByName = async (searchTerm) => {
        try {
            ...
            const response = await axios.get(
                `http://localhost:3000/cards/search?name=${encodeURIComponent(searchTerm)}`
            );
            
            if (response.data.success) {
                if (response.data.data.length > 0) {
                    setCard(response.data.data[0]);
                    navigate(`/view/${response.data.data[0]._id}`);
                } else {
            ...
            setError(error.response?.data?.message || 'Error searching for card');
            setCard(null);
        } finally {
            setLoading(false);
        }
    ...
    if (error) {
        return (
            <div>
                <h1>{error}</h1>
                <button onClick={() => setError(null)}>Search Again</button>
            </div>
        );
    }
